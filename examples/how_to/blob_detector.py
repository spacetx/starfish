...
microscope settings. The table below is a helpful guide for setting the sigma parameters.
...
low intensities that are likely background noise. The spot intensity is calculated based on
``measurement_type`` chosen (e.g. mean or max). One way to set ``threshold`` is to choose a
