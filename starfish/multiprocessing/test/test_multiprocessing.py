...
def test_numpy_array(nitems: int = 10):
    ...
    _start_process_to_test_shmem(
        array_holder=array, decoder=_decode_numpy_array_to_numpy_array, nitems=nitems
    )
...
def test_shmem_numpy_array(nitems: int = 10):
    ...
    array.fill(0)
    _start_process_to_test_shmem(
        array_holder=buffer, decoder=_decode_array_to_numpy_array, nitems=nitems
    )
    for ix in range(nitems):
        assert array[ix] == ix
...
def _decode_imagestack_array_to_numpy_array(shape: Tuple[int, ...], dtype, buffer) -> np.ndarray:
...
def _start_process_to_test_shmem(
    *, array_holder: Any, decoder: Callable[[Any], np.ndarray], nitems: int
) -> None:
    """
    ...
    with multiprocessing.Pool(
        initializer=SharedMemory.initializer, initargs=(array_holder,)
    ) as pool:
