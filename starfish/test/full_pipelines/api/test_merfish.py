...
def test_merfish_pipeline_cropped_data():
    ...
    assert np.allclose(expected_primary_image, primary_image.xarray[5, 0, 0, 40:50, 45:55])
    ...
    assert np.allclose(expected_high_passed, high_passed.xarray[5, 0, 0, 40:50, 45:55])
    ...
    assert np.allclose(expected_deconvolved_values, deconvolved.xarray[5, 0, 0, 40:50, 45:55])
    ...
    assert np.allclose(expected_low_passed, low_passed.xarray[5, 0, 0, 40:50, 45:55])
    ...
    assert np.allclose(expected_scaled_low_passed, scaled_image.xarray[5, 0, 0, 40:50, 45:55])
    ...
    expected_counts = pd.Series(
        [120, 101, 54, 43, 30], index=("nan", "MALAT1", "SRRM2", "FASN", "PRKDC")
    )
    assert np.array_equal(expected_counts.values, result_counts.values)
    assert np.array_equal(expected_counts.index, result_counts.index)
