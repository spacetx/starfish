...
from starfish.core.types import SpotFindingResults, PerImageSliceSpotResults, SpotAttributes
...
def test_saving_spots() -> None:
    ...
    for ax in data.physical_coord_ranges.keys():
        np.testing.assert_equal(data.physical_coord_ranges[ax].to_numpy(), data2.physical_coord_ranges[ax].to_numpy())
