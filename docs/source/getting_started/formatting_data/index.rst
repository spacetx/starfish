...
We provide two types of tools to convert data into SpaceTx-Format. One is a `Bio-Formats`_ writer
...
Second, we provide two classes :py:class:`TileFetcher` and :py:class:`FetchedTile` which enable
users to construct their own parsers that write SpaceTx-Format from their input files. This route
requires a bit of knowledge of coding in python, but has the advantage that once written it should
work for any datasets written in the same way by your microscopy system.
...
.. literalinclude:: /../../starfish/_experiment/builder/providers.py
...
.. literalinclude:: ../../_static/data_formatting_examples/format_iss_breast_data.py
...
.. code-block:: bash
    ...
    mkdir iss/formatted
    python3 docs/source/_static/data_formatting_examples/format_iss_breast_data.py \
        iss/raw/ \
        iss/formatted \
        2
