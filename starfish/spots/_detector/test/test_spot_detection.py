...
def simple_trackpy_local_max_spot_detector() -> TrackpyLocalMaxPeakFinder:
    ...
    return TrackpyLocalMaxPeakFinder(spot_diameter=3, min_mass=0.01, max_size=10, separation=2)


def simple_local_max_spot_detector() -> LocalMaxPeakFinder:
    return LocalMaxPeakFinder(
        min_distance=6, stringency=0, min_obj_area=0, max_obj_area=np.inf, threshold=0
    )
...
def test_spot_detection_with_reference_image(
    ...
    assert np.allclose(
        intensity_table.sum((Axes.ROUND, Axes.CH)).values, expected
