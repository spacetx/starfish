...
def test_imagestack_indexing():
    ...
    expected_shape = OrderedDict(
        [(Axes.ROUND, 4), (Axes.CH, 1), (Axes.ZPLANE, 1), (Axes.Y, 200), (Axes.X, 200)]
    )
    ...
    expected_shape = OrderedDict(
        [(Axes.ROUND, 1), (Axes.CH, 1), (Axes.ZPLANE, 1), (Axes.Y, 200), (Axes.X, 200)]
    )
    assert indexed.shape == expected_shape
    ...
    indexed_stack = stack.sel(
        {Axes.ROUND: 0, Axes.CH: 0, Axes.ZPLANE: 1, Axes.Y: 100, Axes.X: (None, 100)}
    )
    expected_shape = OrderedDict(
        [(Axes.ROUND, 1), (Axes.CH, 1), (Axes.ZPLANE, 1), (Axes.Y, 1), (Axes.X, 100)]
    )
    ...
    expected_shape = OrderedDict(
        [(Axes.ROUND, 5), (Axes.CH, 5), (Axes.ZPLANE, 15), (Axes.Y, 100), (Axes.X, 100)]
    )
    assert indexed_stack.shape == expected_shape

    # index on single x and y
    indexed_stack = stack.sel({Axes.ROUND: 0, Axes.CH: 0, Axes.ZPLANE: 1, Axes.Y: 100, Axes.X: 150})
    expected_shape = OrderedDict(
        [(Axes.ROUND, 1), (Axes.CH, 1), (Axes.ZPLANE, 1), (Axes.Y, 1), (Axes.X, 1)]
    )
    assert indexed_stack.shape == expected_shape

    # Negative indexing
    indexed_stack = stack.sel(
        {Axes.ROUND: 0, Axes.CH: 0, Axes.ZPLANE: 1, Axes.Y: (None, -10), Axes.X: (None, -10)}
    )
    expected_shape = OrderedDict(
        [(Axes.ROUND, 1), (Axes.CH, 1), (Axes.ZPLANE, 1), (Axes.Y, 190), (Axes.X, 190)]
    )
    assert indexed_stack.shape == expected_shape
