...
def test_multiple_tiles_of_same_dtype():
    ...
    expected = np.ones((NUM_ROUND, NUM_CH, NUM_Z, HEIGHT, WIDTH), dtype=np.uint32)
...
def test_int_type_promotion():
    ...
    expected = img_as_float32(np.ones((NUM_ROUND, NUM_CH, NUM_Z, HEIGHT, WIDTH), dtype=np.int32))
...
def test_uint_type_promotion():
    with warnings.catch_warnings(record=True) as warnings_:
        ...
        assert issubclass(warnings_[0].category, DataFormatWarning)
        assert issubclass(warnings_[1].category, UserWarning)
    expected = img_as_float32(np.ones((NUM_ROUND, NUM_CH, NUM_Z, HEIGHT, WIDTH), dtype=np.uint32))
    corner = img_as_float32(np.ones((HEIGHT, WIDTH), dtype=np.uint8))
    expected[0, 0, 0] = corner
    assert np.array_equal(stack.xarray, img_as_float32(expected))
...
def test_float_type_demotion():
    with warnings.catch_warnings(record=True) as warnings_:
        ...
        assert issubclass(warnings_[0].category, DataFormatWarning)
        assert issubclass(warnings_[1].category, UserWarning)
    expected = np.ones((NUM_ROUND, NUM_CH, NUM_Z, HEIGHT, WIDTH), dtype=np.float64)
