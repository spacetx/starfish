    ...
    (
        (
            ...
            (2658848, 4e6),
            {
                ...
                "slicedimage": {"caching": {"directory": "REPLACEME"}},
            },
        ),
        (
            "disabled",
            (0, 0),
            {"validation": {"strict": True}, "slicedimage": {"caching": {"size_limit": 0}}},
        ),
        (
            "limited",
            (1e5, 3e6),
            {
                "validation": {"strict": True},
                "slicedimage": {"caching": {"directory": "REPLACEME", "size_limit": 1e5}},
            },
        ),
    ),
)
...
def test_starfish_config(tmpdir, monkeypatch):
    ...
    setup_config(config, tmpdir, monkeypatch, STARFISH_SLICEDIMAGE_CACHING_SIZE_LIMIT="1")
    assert 1 == StarfishConfig().slicedimage["caching"]["size_limit"]

    config = {"slicedimage": {"caching": {"size_limit": 0}}}
    setup_config(config, tmpdir, monkeypatch, SLICEDIMAGE_CACHING_SIZE_LIMIT="1")
    assert 1 == StarfishConfig().slicedimage["caching"]["size_limit"]
    ...
    setup_config(
        config, tmpdir, monkeypatch, STARFISH_VALIDATION_STRICT=None
    )  # Disable from travis
    assert not StarfishConfig().strict

    setup_config({}, tmpdir, monkeypatch, STARFISH_VALIDATION_STRICT="true")
    assert StarfishConfig().strict

    setup_config({}, tmpdir, monkeypatch, STARFISH_VALIDATION_STRICT="false")
    assert not StarfishConfig().strict
...
def test_starfish_environ_warn_on_duplicate(tmpdir, monkeypatch):
    ...
    with environ(SLICEDIMAGE_CACHING_DIRECTORY="foo", STARFISH_SLICEDIMAGE_CACHING_DIRECTORY="bar"):
        with warnings.catch_warnings(record=True) as warnings_:
